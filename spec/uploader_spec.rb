...
describe Cloudinary::Uploader do
  ...
  it "should support upload_large", :large => true do
    ...
    Tempfile.open([filename, ".bpm"]) do |temp_file|
      ...
      expect(resource["resource_type"]).to eq("raw")
      expect(resource["original_filename"]).to eq(temp_file_filename)

      resource2 = Cloudinary::Uploader.upload_large(temp_file_name,
                                                    :chunk_size => LARGE_CHUNK_SIZE,
                                                    :tags => ["upload_large_tag", TIMESTAMP_TAG],
                                                    :resource_type => "image",
                                                    :use_filename => true,
      ...
      expect(resource2["tags"]).to eq(["upload_large_tag", TIMESTAMP_TAG])
      expect(resource2["resource_type"]).to eq("image")
      expect(resource2["original_filename"]).to eq(filename)
      expect(resource2["original_filename"]).to eq(resource2["public_id"])
      expect(resource2["width"]).to eq(LARGE_FILE_WIDTH)
      expect(resource2["height"]).to eq(LARGE_FILE_HEIGHT)
